...
export default async function handler(req, res) {
  ...
  try {
    ...
    const pageSize = 100;
    const [comments, totals, currentSource, settings] = await Promise.all([list("approved", pageSize, offset), counts(), source(), wallSettings()]);
    const nextOffset = comments.length === pageSize && offset + comments.length < totals.approved ? offset + comments.length : null;
    json(res, 200, {
      comments, nextOffset, total: totals.approved, settings,
